...
bool UdpClient::send( const std::string & input ){
  std::string logline = "Writing to UDP(port=" 
  ...
  input.copy( buff, input.size() );
  short _ss = sendto( socketFd, (const char *) buff, input.size(), MSG_CONFIRM, (const struct sockaddr *)&address, sizeof(address) );
  if( _ss < 0 ){
